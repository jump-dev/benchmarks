    ...
    for current_spin in 1:num_spins
        ...
        if method == "logrise"
            @NLobjective(
                ...
                log(
                    sum(
                        (samples_histo[k, 1] / num_samples) * exp(
                            -(sum(
                                x[i] * nodal_stat[k, i] for i in 1:num_spins
                            )),
                        ) for k in 1:num_conf
                    ),
        ...
        elseif method == "rise"
            @NLobjective(
                ...
                Min,
                sum(
                    (samples_histo[k, 1] / num_samples) *
                    exp(-(sum(x[i] * nodal_stat[k, i] for i in 1:num_spins)))
                    for k in 1:num_conf
                ) + lambda * sum(z[j] for j = 1:num_spins if current_spin != j)
            )
        else
            ...
            @NLobjective(
                ...
                Min,
                sum(
                    (samples_histo[k, 1] / num_samples) * log(
                        1 + exp(
                            -2 *
